...
1. **跨版本回溯/复现问题** ：为老版本创建独立工作区，快速拉起构建与测试，避免污染当前开发目录。
2. **长生命周期分支隔离** ：复杂重构或实验性优化放在独立 worktree，远离日常目录与依赖环境。
3. **大仓多版本并行编译** ：同一 [monorepo](https://zhida.zhihu.com/search?content_id=261557484&content_type=Article&match_order=1&q=monorepo&zhida_source=entity) (monolithic repository)多分支同时编译，复用对象库而非多次 clone，节省磁盘与 I/O。
4. **代码评审/验证专用工作区** ：为某个 PR/提交开临时工作区，拉代码、跑验证，用完即可清理。
